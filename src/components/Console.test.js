...
describe("<Console />", () => {
  ...
  describe("When a command is submitted", () => {
    ...
    it("logs the command", () => {
      ...
      expect(
        store.dispatch.calledWith({ type: "LOG", payload: "fooCommand()" })
      ).to.equal(true, "log action was not called correctly on submit");
    });
    it("logs invalid commands", () => {
      ...
      expect(
        store.dispatch.calledWith({ type: "LOG", payload: "Invalid command" })
      ).to.equal(true, "log action was not called correctly on submit");
    });
    it("calls the move() action", () => {
      ...
      expect(store.dispatch.calledWith({ type: "MOVE" })).to.equal(
        true,
        "MOVE action was not called correctly on submit"
      );
    });
    ...
    it("calls the left() action", () => {
      ...
      expect(store.dispatch.calledWith({ type: "LEFT" })).to.equal(
        true,
        "LEFT action was not called correctly on submit"
      );
    });
    ...
    it("calls the right() action", () => {
      ...
      expect(store.dispatch.calledWith({ type: "RIGHT" })).to.equal(
        true,
        "RIGHT action was not called correctly on submit"
      );
    });
    ...
    it("calls the place() action", () => {
      ...
      expect(
        store.dispatch.calledWith({
          type: "PLACE",
          payload: { x: 1, y: 3, facing: "EAST" }
        })
      ).to.equal(true, "PLACE action was not called correctly on submit");
    });
    ...
    it("calls the report() action", () => {
      ...
      form.simulate("submit");
      expect(
        store.dispatch.calledWith({ type: "LOG", payload: "0, 0, NORTH" })
      ).to.equal(true, "Report action was not called correctly on submit");
    });
  });
});
